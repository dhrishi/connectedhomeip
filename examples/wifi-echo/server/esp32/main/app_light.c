...
esp_err_t app_light_get_from_nvs(void)
{
    nvs_handle handle;
    esp_err_t err = nvs_open_from_partition("nvs", "default_light", NVS_READWRITE, &handle);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "NVS open failed");
        return ESP_FAIL;
    }
    nvs_get_u8(handle, NVS_KEY_RED, &g_red);
    nvs_get_u8(handle, NVS_KEY_GREEN, &g_green);
    nvs_get_u8(handle, NVS_KEY_BLUE, &g_blue);
    size_t switch_size = sizeof(g_switch);
    nvs_get_blob(handle, NVS_KEY_SWITCH, &g_switch, &switch_size);
    nvs_close(handle);
    ESP_LOGI(TAG, "Read from NVS. %d %d %d %d", g_red, g_green, g_blue, g_switch);
    return ESP_OK;
}

esp_err_t app_light_save_to_nvs(void)
{
    nvs_handle handle;
    esp_err_t err = nvs_open_from_partition("nvs", "default_light", NVS_READWRITE, &handle);
    if (err != ESP_OK) {
    ...
    nvs_set_u8(handle, NVS_KEY_RED, g_red);
    nvs_set_u8(handle, NVS_KEY_GREEN, g_green);
    nvs_set_u8(handle, NVS_KEY_BLUE, g_blue);
    nvs_set_blob(handle, NVS_KEY_SWITCH, &g_switch, sizeof(g_switch));
    nvs_close(handle);
